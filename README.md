...
В `.env` укажите `TASK_VERSION=2` (вариант задания — навыки пользователей).
...
Демо-пользователи (пароль `demo12345`): `alice@example.com`, `bob@example.com`, `carol@example.com`.
...
Проект доступен по адресу [http://localhost:8000](http://localhost:8000). Главная страница — список проектов `/projects/list/`.

Для локальной проверки без Docker можно временно добавить в `.env` строку `USE_SQLITE=True` (для сдачи проекта используйте PostgreSQL в контейнере, как в задании).
